      ...
      <div>
        ...
        <div class="novel_div">
          ...
          <div
            class="newNovel"
            id="newNovel"
            @click="novelEvent($event, 'newNovel', 'release')"
          >
            신작
          </div>
          <div
            class="payNovel"
            id="payNovel"
            @click="novelEvent($event, 'payNovel', 'bought')"
          >
            구매↑
          </div>
          <div
            class="starNovel"
            id="starNovel"
            @click="novelEvent($event, 'starNovel', 'review')"
          >
            리뷰↑
          </div>
        </div>
      ...
      <div class="side_genre_group">
        ...
        <div class="genre_group" v-if="hiddenData">
          ...
          >
            <div
              class="genre_select"
              @click="categoryEvent(i, $event, cate.CATE_CODE)"
            >
...
export default {
  ...
  created() {
    ...
    if (this.$route.params.STORE != undefined) {
      this.$store.commit("currentServiceCng", "S");
      this.storeCode = this.$route.params.STORE;
    }
  },
  mounted() {
  ...
  methods: {
    ...
    getNovelListFirst() {
      ...
      axios.post("/api/store/getNovelList", this.search).then((result) => {
        if (result.data == "err") {
    ...
    // 카테고리 불러오기
    getCateList() {
      axios.get("/api/store/getCateList").then((result) => {
        if (result.data == "err") {
